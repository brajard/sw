...
#Model definition
model = Sequential()
model.add(Conv2D(32, (3, 3), activation='relu', padding='same', input_shape=(ny, nx, npar)))
...
#Training
nn.fit(X_train, y_train, epochs=500, batch_size=1)

y_predict = nn.predict(X_test)
if PLOT:
	plt.plot(y_test,y_predict,'.')
	plt.show()
nn.save('../data/model_'+param+'par-im.pkl')
